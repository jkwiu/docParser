...
for filename in os.listdir('code'):
    ...
    if filename.endswith('.cs'):
        with open(os.path.join('./code/', filename), encoding='utf8') as f:
            lines = f.read().split("\n")
            for i, line in enumerate(lines):
                ...
                if len(keyword) > 0:
                    isFunction = True
                    # Function name 값
                    funcNameKeywordArr = re.findall(
                        funcNameExpPattern, keyword[0])
                    isGgwal = True
            ...
            for i, line in enumerate(lines):
                if i > initialStartLine:
                    paramSummaryKeywordArr = re.findall(
                        paramSmryExpPattern, line)
            ...
            for i in range(0, len(dicNameArr)):
                ...
                for j in range(0, len(dic[dicNameArr[i]]['table'])):
                    if len(paramSummaryArr) > 0:
                        dic[dicNameArr[i]
                            ]['table'][j]['Param summary'] = paramSummaryArr[0]
            ...
            for name in dicNameArr:
                ...
                table = doc.add_table(
                    rows=rowLength, cols=3, style='TableGrid')
                # 기본 테이블 설정
                ...
                make_rows_bold(
                    table.rows[0], table.rows[1], table.rows[2], table.rows[3])
